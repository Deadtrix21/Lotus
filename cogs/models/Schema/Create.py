...
from marshmallow import (post_dump, post_load, pre_dump, pre_load, pprint)
import arrow
...
class Account():
    def __init__(self, password, email, created_at):
        ...
        self.email = email
        self.created_at = arrow.get(created_at) if created_at != None else arrow.now().isoformat()
...
class Staff():
    def __init__(self, enable=False, category="None"):
        self.enable = enable
...
class Final():
    def __init__(self, discord):
        ...
        self.Exp = Exp()
        self.Account = Account(created_at=arrow.now().isoformat(), password="exmaple", email="example@outlook.com")
