...
import pymongo, marshmallow, discord
from numpy.random import choice
from .Schema.Create import UserSchema, Final
...
class Manager():
    ...
    def __get_database(self, arg:str) -> pymongo.MongoClient:
        arg = arg.lower()
        switch_case = {
            "user" : self.__db["NightmareFever"]["Users"],
            "shop" : self.__db
        }
...
# UI
    ...
    async def pay_ui(self, ctx, Member, value, word):
        Embed = discord.Embed(
            title= f"{word} Transaction",
            description="",
            color= 0x000c30
        )
        Embed.add_field(name="Entry", value=f"{ctx.author.mention} paid {Member.mention}")
...
# functions
    ...
    async def add_money_auto(self, ctx, AmountGained, ExpGained):
        ...
        res = await self.handle_exp_money(self.__Account__Load.load(query), AmountGained, ExpGained)
    ...
    async def add_money_admin(self, ctx, Member, AmountGained):
        query = await self.user_database_query(query={'Discord_Id': Member.id})
        res = await self.handle_exp_money(self.__Account__Load.load(query), AmountGained, 0)
        res = self.__Account__Load.dump(res)
        await self.user_database_update(query, res)
